jQuery.noConflict()(function($) {
    ...
    var initLoadMoreFunction = function() {
        var body = $('body');
        var page_num = parseInt(cherie_pagination_data.startPage) + 1;
        var max_pages = parseInt(cherie_pagination_data.maxPages);
        var next_link = cherie_pagination_data.nextLink;

        var container = cherie_pagination_data.container;
        var $container = $(container);
        var container_has_isotope = false;


        var last_btn_text = cherie_pagination_data.button_text_no_post;
        ...
        if($container.hasClass('fl--blog-post-div')){
            last_btn_text = cherie_pagination_data.button_text_no_post;
        }
        // Last Page Woo Text
        if($container.hasClass('fl--work--wrapper')){
            last_btn_text = cherie_pagination_data.button_text_no_work;
        }
        //Last Page Portfolio
        if($container.hasClass('fl--portfolio-wrap')){
            last_btn_text = cherie_pagination_data.button_text_no_portfolio;
        }
            ...
            if (!$(this).hasClass('loading') && !$(this).hasClass('load_more_disable') && page_num <= max_pages) {
                $.ajax({
                    ...
                    beforeSend: function() {
                        ...
                        $button.find('span').text(cherie_pagination_data.button_loading);
                    },
                    complete: function(content) {
                        $button.find('span').text(cherie_pagination_data.button_text);
